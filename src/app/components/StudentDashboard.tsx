      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_320px]">
          <section className="rounded-3xl border border-violet-100 bg-white p-6 shadow-sm sm:p-8" aria-labelledby="student-title">
            <span className="eyebrow">Progresso local</span>
            <h1 id="student-title" className="mt-3 text-3xl font-bold tracking-tight text-slate-900 sm:text-4xl">Seu percurso</h1>
            ...
            <div className="mt-8 rounded-2xl bg-gradient-to-br from-violet-700 to-blue-600 p-6 text-white">
              <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
                <div>
                  <div className="flex items-center gap-2 text-sm text-white/75"><BookOpen className="h-4 w-4" /> Percurso demonstrativo</div>
                  <h2 className="mt-2 text-2xl font-bold">{demoCourse.title}</h2>
                  <p className="mt-1 text-sm font-semibold text-white/85">{demoCourse.instructor.name}</p>
                ...
                <button className="focus-ring inline-flex min-h-11 shrink-0 items-center justify-center gap-2 rounded-xl bg-white px-5 py-3 font-bold text-violet-700 hover:bg-violet-50" onClick={() => onOpenLesson(nextLesson.id)}>
                  <Play className="h-4 w-4 fill-current" /> {isComplete ? "Rever percurso" : completed > 0 ? "Continuar aprendendo" : "Começar primeira etapa"}
                </button>
              </div>
              <div className="mt-6" aria-label={`${progress}% do curso concluído`}>
                <div className="mb-2 flex justify-between text-sm"><span>{completed} de {demoCourse.lessons.length} etapas</span><strong>{progress}%</strong></div>
            ...
            <div className="mt-8 space-y-3">
              <div className="flex items-center justify-between gap-4">
                <h2 className="text-xl font-bold text-slate-900">Etapas do percurso</h2>
                {completed > 0 && <button className="focus-ring inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-semibold text-slate-500 hover:bg-slate-100 hover:text-slate-900" onClick={onReset}><RotateCcw className="h-3.5 w-3.5" /> Recomeçar demonstração</button>}
              </div>
          ...
          <aside className="space-y-6">
            <div className={`rounded-3xl border p-6 ${isComplete ? "border-emerald-200 bg-emerald-50" : "border-slate-200 bg-white"}`}>
              {isComplete ? <><CheckCircle2 className="h-8 w-8 text-emerald-600" /><h2 className="mt-4 text-xl font-bold text-slate-900">Percurso concluído</h2><p className="mt-2 text-sm leading-6 text-slate-600">Você finalizou todas as etapas da demonstração neste navegador.</p></> : <><BookOpen className="h-8 w-8 text-violet-600" /><h2 className="mt-4 text-xl font-bold text-slate-900">Seu percurso começa na primeira etapa</h2><p className="mt-2 text-sm leading-6 text-slate-600">Conclua as etapas em ordem para acompanhar seu progresso.</p></>}
            </div>
